...
import { getPatientNotifications } from "../../../services/patientService.js";
...
async function handleNotificaionCount(id) {
  notifications = await getPatientNotifications(id);
  notifications = notifications.filter(
    (notification) => notification.isReadPatient == false
  );
...
function displayNotifications(notifications) {
  ...
  notifications.forEach((notification) => {
    ...
    archiveButton.addEventListener("click", async () => {
      await markNotificationAsRead(notification.id, "patients");
